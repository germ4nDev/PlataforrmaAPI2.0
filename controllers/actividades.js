...
//     return res.status(201).json({
//       ok: true,
//       message: 'La actividad se creo correctamente',
//       actividad: actividad,
//     });
//   } catch (err) {
...
const actividadesService = require("../services/actividades.service"); // Ajustar ruta
...
const getActividadById = async (req, res = response) => {
  try {
    const actividad = await actividadesService.obtenerActividadPorId(req.params.id);
...
const getActividadByCodeApp = async (req, res = response) => {
  try {
    const actividades = await actividadesService.obtenerActividadesPorApp(req.params.id);
    return res.status(200).json({
      ok: true,
      actividades,
    });
...
const getActividadByCodeSuite = async (req, res = response) => {
  try {
    const actividades = await actividadesService.obtenerActividadesPorSuite(req.params.id);
    return res.status(200).json({
      ok: true,
      actividades,
    });
...
const getActividadByCodeModulo = async (req, res = response) => {
  try {
    const actividades = await actividadesService.obtenerActividadesPorModulo(req.params.id);
    return res.status(200).json({
      ok: true,
      actividades,
    });
...
const createActividad = async (req, res = response) => {
  try {
    const actividadDB = await actividadesService.crearActividad(req.body);
    return res.status(201).json({
      ok: true,
      actividad: actividadDB,
    });
...
const updateActividad = async (req, res = response) => {
  try {
    ...
    const actividadActualizada = await actividadesService.actualizarActividad(codigoActividad, data, usuarioId);
...
const deleteActividad = async (req, res = response) => {
  try {
    const actividadEliminada = await actividadesService.eliminarActividad(req.params.id);
